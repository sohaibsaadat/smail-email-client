...
const { filteredEmails } = useOutletContext();
  ...
  // All Mail filter (is_deleted === false included to exclude trash items)

  // Helper Function: HTML tags ko strip karke clean text nikalne ke liye
  ...
  return (
    <div className="w-full">
      {filteredEmails.map((email) => {
        ...
        // Dynamic Display Name: Sent mails ke liye Recipient, baaki ke liye Sender
        const isSentFolder = email.folder === "Sent";
        const displayName = isSentFolder
          ? `To: ${email.recipients?.[0]?.name || email.recipients?.[0]?.email || "Unknown"}`
          : email.sender || "Unknown";

        return (
          <div
            key={email.id}
            onClick={() => (isSentFolder ? handleEmailClick(email.id) : readMail(email.id))}
          ...
          >
            ...
            {/* Middle Section: Sender/Recipient, Folder Badge, Subject, & Body Preview */}
            <div className="flex-1 flex lg:flex-row flex-col items-start lg:items-center gap-1 lg:gap-3 overflow-hidden min-w-0">
              ...
              {/* Folder Tag Badge */}
              <span className="px-2 py-0.5 text-[10px] font-medium bg-gray-200 text-gray-700 rounded-md whitespace-nowrap">
                {email.folder || "Inbox"}
              </span>
