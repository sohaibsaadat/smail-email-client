...
const Sent = ({ open }) => {
  const { emails, selectedEmails, handleSelectEmail, handleEmailClick, starredMail, formatEmailDate } = useEmail();
  const { sliceNumber } = useResponsiveSlice();
const { filteredEmails } = useOutletContext();

  // Sent folder filter (is_deleted === false included for trash isolation)
  const sentEmails = emails.filter((email) => email.folder === 'Sent' && email.is_deleted === false);

  // Helper Function: HTML tags ko strip karke plain text nikalne ke liye
  ...
  return (
    <div className="w-full">
      {filteredEmails.map((email) => {
        ...
        // Recipient display name extraction with fallback handling
        const recipientName =
          email.recipients && email.recipients.length > 0
            ? email.recipients[0]?.name || email.recipients[0]?.email || "To: Unknown"
            : "To: Unknown";
        ...
        return (
          ...
          >
            ...
            {/* Middle Section: Recipient, Subject, & Body Preview */}
