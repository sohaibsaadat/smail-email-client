...
const Inbox = ({ open }) => {
  const { emails, selectedEmails, handleSelectEmail, starredMail, readMail, formatEmailDate } = useEmail();
  const { sliceNumber } = useResponsiveSlice();
const { filteredEmails } = useOutletContext();

  // Helper Function: HTML ko plain text mein convert karne ke liye (List view optimization)
